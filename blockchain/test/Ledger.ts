...
describe("Ledger contract", function () {
    ...
    describe("Account Creation", function() {
       ...
       it ("Should be able to add register receiver",async function(){
        ...
        it("Users should be able to list registered receivers",async function () {
            const { ledger , addr1,addr2,addr3 } = await loadFixture(deployLedgerFixture);
            let receiverList = await ledger.getReceivers();
            ledger.connect(addr1).createAccount(0);
            expect(await ledger.connect(addr1).isRegistered()).to.be.true;
            ledger.connect(addr2).createAccount(0);
            ledger.connect(addr3).createAccount(0);
            ledger.connect(addr1).registerReceiver(addr2.address,"kullanıcı 1");
            ledger.connect(addr1).registerReceiver(addr3.address,"kullanıcı 2");
            console.log( await ledger.connect(addr1).getReceivers());
